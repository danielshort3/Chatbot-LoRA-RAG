...
def _ensure_model() -> None:
    """Ensure ``LLM_NAME`` is available by running ``ollama pull``.

    If the model is already present locally the command is a no-op. Any
    failure is surfaced with a helpful ``RuntimeError`` so callers get a clear
    message instead of a 404 from later generate requests.
    """
    ...
    try:
        ...
        raise RuntimeError(f"Failed to pull model '{LLM_NAME}'") from exc
